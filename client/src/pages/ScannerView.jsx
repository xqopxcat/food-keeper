...
import { lookupByBarcode, estimateShelfLife, subscribePush, sendTestPush } from '../api.js';
import { inferDefaultsFromProduct } from '../inferDefaults.js';
import { foodOptions, unitOptions } from '../constants';
...
const ScannerView = () => {
  ...
  async function handleAddToInventory() {
    ...
    try {
      ...
      const response = await estimateShelfLife(payload);
    ...
    <div>
      ...
      {barcode && (<div style={{ marginTop:12 }}>
        ...
        <div style={{ display:'flex', gap:8, marginTop:10, flexWrap: 'wrap' }}>
          ...
          <button onClick={() => handleEstimate(true)} disabled={!readyForEstimate}>
            💾 估算並入庫
          </button>

          <button 
            onClick={handleAddToInventory}
            disabled={!facts.itemKey}
            style={{
              ...
              padding: '8px 16px',
              borderRadius: '6px',
              cursor: facts.itemKey ? 'pointer' : 'not-allowed',
              opacity: facts.itemKey ? 1 : 0.6
            }}
          >
            📦 加入庫存
          </button>
